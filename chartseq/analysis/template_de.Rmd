...
We also remove duplicate reads that form problematic stacks.

```{r}
library(csaw)
param <- readParam(minq=10, pe="both", dedup=TRUE, max.frag=1000)
bamdir <- "../bam"
...
```{r}
win.data <- normFactors(bin.data, se.out=win.data)
normfacs <- win.data$norm.factors
data.frame(File=fnames, Total=win.data$totals, Norm.Factor=normfacs)
```
...
```{r}
labels <- paste0(sub("271", "", gsub("_(.)[^_]+", "\\1", Group)), ".", Batch)
wadjc <- calculateCPM(win.data, log=TRUE, prior.count=2)
par(mfrow=c(2,2))
for (top in c(500, 2000, 5000, 20000)) {
    plotMDS(wadjc, labels=labels, top=top, main=top)
}
```
